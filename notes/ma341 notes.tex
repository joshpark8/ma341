...
\begin{document}
    ...
    \begin{theorem}[(Location of Roots)]
        suppose $f:[a,b]\to\bbR$ continuous and either $f(a)<0<f(b)$ \tun{or} $f(a)>0>f(b)$.
    ...
    \begin{definition}
        Let $f:A\to\bbR$. $f$ is \tun{uniformly continuous} on $A$ if $\forall \varepsilon > 0,$ $\exists \delta > 0$ such that $\left|f(x)-f(y)\right|<\varepsilon$ if $|x-y|<\delta$, $\forall x,y\in A$.
    \end{definition}
    ...
    \end{proof}
\begin{definition}
    If $f:A\to\bbR$ and $B\supseteq A$, then $g:B\to\bbR$ is an \tun{extension} of $f$ if $g(x)=f(x) \forall x\in A$
\end{definition}
\begin{definition}
