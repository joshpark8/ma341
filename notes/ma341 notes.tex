...
\begin{document}
    ...
    The following statements are equivalent to ii) below. \vspace{-1em} \begin{enumerate}
        \item[ii)'] If $z>s$, then $z$ is \uline{not} an upper bound for $A$.
    ...
    \begin{proof}
        Assume $f$ is \uline{not} bounded. That is, for each $n\in\N, \exists x_n\in[a,b]$ such that $|f(x_n)|\geq n$. \\
    ...
    \begin{theorem}[(Location of Roots)]
        suppose $f:[a,b]\to\bbR$ continuous and either $f(a)<0<f(b)$ \uline{or} $f(a)>0>f(b)$.
    ...
    \begin{definition}
        Let $f:A\to\bbR$. $f$ is \uline{uniformly continuous} on $A$ if $\forall \varepsilon > 0,$ $\exists \delta > 0$ such that $\left|f(x)-f(y)\right|<\varepsilon$ if $|x-y|<\delta$, $\forall x,y\in A$.
    \end{definition}
    ...
    \end{theorem}
    \begin{proof}
        Suppose $f$ \uline{not} uniformly conitinuous, so that $\exists \xn, (u_n)$ in $[a,b]$ and $\varepsilon_0 >0$ such that $|x_n-u_n|\to 0$ and $|f(x_n)-f(u_n)|\geq \varepsilon_0$. By the Bolzano-Weierstrauss Theorem, $\exists \xnkp$ such that $\xnk\to z\in[a,b]$.\\
    ...
    \end{proof}
\begin{definition}
    If $f:A\to\bbR$ and $B\supseteq A$, then $g:B\to\bbR$ is an \uline{extension} of $f$ if $g(x)=f(x) \forall x\in A$
\end{definition}
\begin{definition}
    If $f$ and $g$ are continuous on $A$ adn $B$, respectively, we say $g$ is a \uline{continuous extension} of $f$.
\end{definition}
